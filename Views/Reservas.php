    ...
    <form method="get" class="table-responsive" style="max-height: 60vh; overflow-y: auto;">
        ...
        <table class="table table-striped">
            <thead class="thead-dark" style="position: sticky; top: 0; z-index: 1000">
                <tr>
                    <th colspan="2"><input type="search" class="form-control" name="code"
                            placeholder="Filtrar por código" value="<?= isset($_GET["code"]) ? $_GET["code"] : "" ?>">
                    </th>
                    <th colspan="2"><input type="search" class="form-control" name="client"
                            placeholder="Filtrar por cliente"
                            value="<?= isset($_GET["client"]) ? $_GET["client"] : "" ?>"></th>
                ...
                if (isset($reserves) && $reserves->num_rows > 0) {
                    while ($row = $reserves->fetch_assoc()) {
                        ...
                        <tr>
                            ...
                            <td>
                                <button type='button' class='btn btn-danger cancel-reserve' data-id='<?= $row['ID'] ?>'
                                    data-bs-toggle='modal' data-bs-target='#cancelModal'>Cancelar</button>
                            </td>
                            <td>
                                <button type='button' class='btn btn-success confirm-reserve' data-id='<?= $row['ID'] ?>'
                                    data-bs-toggle='modal' data-bs-target='#confirmModal'>Confirmar</button>
                            </td>
                        </tr>
                        <?php
                    }
